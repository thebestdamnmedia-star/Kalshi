...
// FILTER 1: skip if resolving within 3 minutes (expiration noise)
...
// FILTER 2: skip near-certain outcomes
...
// Track big moves on flat BTC separately
...
setStatus('Done. '+wins.length+' up moves, '+losses.length+' down moves, '+bigMoves.length+' big moves on flat BTC.');
...
html+='<div class="row"><span class="label">Big moves (20c+) on flat BTC:</span><span class="val amber">'+bigMoves.length+'</span></div>';
html+='</div>';

// BTC BUCKET TABLE
html+='<div class="card"><div class="sec">BTC 30s Move vs Contract Move (filtered)</div>';
