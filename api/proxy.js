// Kalshi Trading Proxy
// Handles: Kalshi (public + signed), Polygon (BTC/ETH bars), Upstash (storage)
// Format: CommonJS (no ES module config needed)
...
return res.status(200).json({ ok: true, result: data[0]?.result });
} catch (err) {
return res.status(500).json({ error: true, source: ‘upstash’, message: err.message });
}
}

// ── Polygon.io (for BTC/ETH price bars) ──────────────────────────────────
if (body._polygon) {
try {
const url = body.url;
...
// ── Kalshi (public + signed) ─────────────────────────────────────────────
// body._kalshi: true
// body.url: full Kalshi URL
// body.method: ‘GET’ (default), ‘POST’, ‘DELETE’
// body.payload: optional JSON body for POST/DELETE
// body.signed: true to use RSA-PSS auth (required for portfolio/orders/balance)
if (body._kalshi) {
try {
const url = body.url;
...
```
  ...
  if (signed) {
    if (!KALSHI_KEY_ID || !KALSHI_PRIVATE_KEY) {
      return res.status(401).json({ error: true, source: 'kalshi', message: 'KALSHI_KEY_ID and KALSHI_PRIVATE_KEY env vars required for signed requests' });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: true, source: 'kalshi', message: 'Failed to sign: ' + sigErr.message });
    }
  ...
  if (!r.ok) {
    return res.status(r.status).json({
      ...
      message: data?.error?.message || data?.message || text.slice(0, 200),
      signed
    });
  }
  return res.status(200).json(data);
